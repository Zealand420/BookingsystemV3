...
function ChangingRoomPage() {
  const [facilities, setFacilities] = useState([]);
  const [filteredFacilities, setFilteredFacilities] = useState([]);
  const [statusFilter, setStatusFilter] = useState("All");
  const [dommerFilter, setDommerFilter] = useState("All");
  const [kønFilter, setKønFilter] = useState("All");
    ...
    const fetchFacilities = async () => {
      try {
        const facilitiesQuery = query(
          ...
          where("Type", "in", ["Omklædningsrum", "Dommer"])
        );
  ...
  useEffect(() => {
    ...
    if (dommerFilter !== "All") {
      filtered = filtered.filter((facility) => facility.Dommer === dommerFilter);
    }

    if (kønFilter !== "All") {
      filtered = filtered.filter((facility) => facility.Køn === kønFilter);
    }
  ...
  }, [statusFilter, dommerFilter, kønFilter, typeFilter, facilities]);
          ...
          <div className="row align-items-center py-3 ms-2 me-2">
            ...
            <div className="col-9 d-flex justify-content-end align-items-center">
              ...
              <select
                ...
                style={{ width: "150px" }}
                value={dommerFilter}
                onChange={(e) => setDommerFilter(e.target.value)}
              >
                <option value="All">Dommer?</option>
              ...
              <select
                ...
                style={{ width: "150px" }}
                value={kønFilter}
                onChange={(e) => setKønFilter(e.target.value)}
              >
...
export default ChangingRoomPage;
